// This file is used to bootstrap the Flutter application
// It helps to ensure proper initialization even if flutter.js fails to load initially

// Helper function to check if flutter.js has loaded
function isFlutterInited() {
  return typeof window._flutter !== 'undefined';
}

// Helper function to retry loading flutter.js
function retryLoadFlutter(callback, maxRetries = 5, retryInterval = 1000) {
  let retries = 0;
  
  function attemptLoad() {
    if (isFlutterInited()) {
      // Flutter is loaded, execute callback
    ...
    if (retries >= maxRetries) {
      console.error('Failed to load Flutter after multiple attempts');
      // Try to show a helpful message to the user
      const loadingScreen = document.getElementById('loading-screen');
      if (loadingScreen) {
        const loadingDiv = loadingScreen.querySelector('.loading');
        if (loadingDiv) {
          const errorMessage = document.createElement('p');
          errorMessage.style.color = '#FF0000';
          errorMessage.textContent = 'Failed to load the application. Please try refreshing the page.';
          loadingDiv.appendChild(errorMessage);
          
          const refreshButton = document.createElement('button');
          refreshButton.textContent = 'Refresh Page';
          refreshButton.style.padding = '10px 15px';
          refreshButton.style.backgroundColor = '#1E40AF';
    ...
    retries++;
    console.log(`Retry ${retries} to load Flutter...`);
...
// Initialize Flutter once the page loads
...
_flutter.loader.load({
  serviceWorkerSettings: {
    serviceWorkerVersion: "3809300614"
  }
});
